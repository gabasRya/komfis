...
function createLayer(
  ...
  if (color === layerExplanations["Crust"].color) {
    // Jika ini adalah CRUST → gunakan texture
    mat = new THREE.MeshStandardMaterial({
      map: earthTexture,
      side: THREE.DoubleSide,
    });
  } else {
    // Layer lain tetap menggunakan warna biasa
    mat = new THREE.MeshStandardMaterial({
      color,
      side: THREE.DoubleSide,
    });
...
for (let i = 1; i < layersConfig.length; i++) {
  ...
  earthGroup.add(mesh);
  interactableObjects.push(mesh); // Tambahkan ke daftar interaktif // --- Buat Label Teks ---
  ...
  label.visible = false;
  earthGroup.add(label); // --- Buat Garis Penunjuk ---
...
// ============= STARFIELD REALISTIS / BINTANG BULAT SOFT =============

// 1. Buat texture bintang berupa lingkaran gradient
...
const starMaterial = new THREE.PointsMaterial({
  ...
  depthWrite: false,   // bikin glow lebih soft
});
...
function onMouseClick(event) {
  ...
  raycaster.setFromCamera(mouse, camera); // Ganti intersectObjects dengan array baru yang berisi semua objek yang dapat diklik
  ...
  if (intersects.length > 0) {
    ...
    if (clickedObject.userData.layerName === "Crust") {
      isExpanded = !isExpanded; // Segera set visibilitas label dan garis setelah klik
...
function animate() {
  ...
  if (isExpanded) {
    ...
    crustLabelObject.position.set(LABEL_FINAL_X, crustYTarget, 0); // P1: Titik dekat permukaan Crust (misal 45 derajat)
    ...
    const crustP2 = new THREE.Vector3(LABEL_FINAL_X - 0.1, crustYTarget, 0); // Update garis Crust (Garis Miring Saja)

    crustLine.geometry.setFromPoints([crustP1, crustP2]);
    crustLine.geometry.attributes.position.needsUpdate = true; // --- 2. LAPISAN DALAM (Mantle, Outer Core, Inner Core) ---

    layerLabels.forEach((item, index) => {
      const layerIndex = index + 1;
      const currentLayerX = item.initialX; // Y target untuk label
      ...
      const p1 = new THREE.Vector3(currentLayerX, startYOffset, 0); // P2: Titik belok horizontal
      ...
      const p3 = new THREE.Vector3(LABEL_FINAL_X - 0.1, yTarget, 0); // Update posisi label

      item.label.position.set(LABEL_FINAL_X, yTarget, 0); // Update garis (3 titik: P1 -> P2 -> P3)
